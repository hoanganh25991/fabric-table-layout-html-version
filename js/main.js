...
exports.default = _vue2.default.directive('longpress', {
	bind: function bind() {},
	update: function update(val, oldVal) {
		console.log('longpress', val, oldVal);
	}
});

},{"vue":5}],7:[function(require,module,exports){
'use strict';
...
var _DirectiveLongPress = require('./DirectiveLongPress');

var _DirectiveLongPress2 = _interopRequireDefault(_DirectiveLongPress);
...
exports.default = _vue2.default.extend({
	...
	ready: function ready() {
		...
		(0, _hammerjs2.default)(vm.$els.vailo).on('press', function (e) {
			console.log(e);
	...
	methods: {
		deleteLayout: function deleteLayout() {
			var vm = this;
			var index = this.layouts.indexOf(vm.layout);

			if (index > -1) {
				this.layouts.splice(index, 1);
				var closestLayout = index;
				if (index > 0) {
		...
		renameLayout: function renameLayout() {
...
},{"./DirectiveLongPress":6,"hammerjs":1,"jquery":2,"vue":5}],8:[function(require,module,exports){
"use strict";
...
exports.default = _vue2.default.extend({
	...
	watch: {
		"newTableName": function newTableName(val, oldVal) {
			if (val) {
	...
	ready: function ready() {
		...
		var canvasId = this.$els.canvas.id;

		var canvas = new fabric.Canvas(this.$els.canvas.id);

		//set width height of canvas
		// let width = Math.floor($(".canvas-container").width());
		var canvasDiv = (0, _jquery2.default)(".canvas-container");
		var width = Math.floor(canvasDiv.width());
		var height = Math.floor(canvasDiv.height());
		...
		if (this.layout.canvas) {
			var canvasObj = vm.getCanvasObj(this.layout.canvas);
		...
		canvas.on('object:longpress', function (e) {
			var table = e.target;
			var tableName = table.item(1).text;
	...
	methods: {
		getCanvasObj: function getCanvasObj(json) {
			var vm = this;
			...
			if (Array.isArray(json)) {
				var _canvasTemp = new fabric.Canvas();
				...
				try {
					for (var _iterator = json[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
						var tableInfo = _step.value;


						var table = this.createFabricTable(tableInfo);

						_canvasTemp.add(table);
					}
				} catch (err) {
				...
				canvasObj = _canvasTemp.toObject();
		...
		relativePosition: function relativePosition() {
			var canvasSize = this.layout.canvasSize;
		...
		notifyTableEvent: function notifyTableEvent(eventName) {
			...
			if (_f.isArray(eventName)) {
				...
				try {
					for (var _iterator2 = eventName[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
						var singleEventName = _step2.value;
		...
		createTable: function createTable(tableName) {
			...
			if (this.layout.name == this.selectedLayout.name) {
				...
				var table = this.createFabricTable(this.tableSizeDefault);
		...
		// 		width: "0.20"
		// },
		createFabricTable: function createFabricTable(tableInfo) {
			//store ref
			var vm = this;
			var canvasSize = vm.layout.canvasSize;
			//text
			...
			//table
			var table = new fabric.Group([shape, text], tableOptions);
		...
		renameTable: function renameTable() {
			var table = this.selectedLayout.canvas.getActiveObject();
			var txt = table.item(1);
...
},{"jquery":2,"vue":5}],9:[function(require,module,exports){
"use strict";
...
exports.default = _vue2.default.extend({
	...
	watch: {
		modifyTableInfo: function modifyTableInfo(val, oldVal) {
			var vm = this;
			...
			var tableProp = val.prop;

			if (tableProp == 'shape') {
				var shape = table.item(0);
				var txt = table.item(1);
			...
			var newVal = mapTableProp[tableProp];
			console.log(newVal);
			//update value
	...
	methods: {
		...
		dispatchExportLayouts: function dispatchExportLayouts() {
	...
	events: {
		...
		'update-table-info': function updateTableInfo() {
...
},{"hammerjs":1,"jquery":2,"vue":5}],10:[function(require,module,exports){
"use strict";
...
_vue2.default.directive('ti-gesture', {
	params: ['modify-table-info'],
	bind: function bind() {},
	update: function update() {
		console.log(this);
		console.log(this.el);

		var intervalUpdateTable = {};

		var vd = this;

		var modifyTableInfo = JSON.parse(vd.params.modifyTableInfo);

		(0, _hammerjs2.default)(this.el).on('press', function (e) {
			intervalUpdateTable = setInterval(function () {
...
var vm = new _vue2.default({
	...
	props: {
		...
		tableEvent: {
			default: function _default() {
				return ["object:selected", "object:scaling", "object:moving", "object:rotating"];
			}
		},
		newTableName: {
			default: ""
	...
	data: {
		...
		// url: 'layouts',
		url: 'https://tinker.press/fabric-table-layout/layouts'
	},
	...
	methods: {
		...
		getLayoutName: function getLayoutName() {
			...
			var layout = { name: newLayoutName, canvasId: Date.now(), canvasSize: {} };
		...
		exportLayouts: function exportLayouts() {
			...
			try {
				var _loop = function _loop() {
					...
					var canvasObj = layout.canvas.toObject(_this.relativePosition(canvasSize));
					console.log("export relative canvasObj|layout : " + layout.name, canvasObj);
					layout.canvas = JSON.stringify(canvasObj);
				};

				for (var _iterator2 = layouts[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
					_loop();
				}
			} catch (err) {
				_didIteratorError2 = true;
			...
			//push to server
			// this.url = "save";
			this.url = 'https://tinker.press/fabric-table-layout/save';
		...
		broadcastUpdateTableInfo: function broadcastUpdateTableInfo() {
...
window.cc = (0, _jquery2.default)('.canvas-container');

},{"./components/LayoutInfo":7,"./components/LayoutTable":8,"./components/TableInfo":9,"hammerjs":1,"jquery":2,"vue":5,"vue-resource":4}]},{},[10]);
